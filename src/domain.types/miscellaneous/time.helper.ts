...
// import { DateStringFormat, DurationType } from "";
...
export class TimeHelper {
    ...
    static formatTimeTo_AM_PM(timeString: { split: (arg0: string) => [any, any]; }) {
        const [hourString, minute] = timeString.split(":");
        const hour = +hourString % 24;
        return (hour % 12 || 12) + ":" + minute + (hour < 12 ? " AM" : " PM");
    }

    static timeDelay = (ms: number) => new Promise(res => setTimeout(res, ms));
