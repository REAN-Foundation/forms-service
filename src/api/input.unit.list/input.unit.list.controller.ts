...
export class InputUnitListController extends BaseController {
    _service: InputUnitListService = Injector.Container.resolve(InputUnitListService);
    ...
    create = async (request: express.Request, response: express.Response) => {
        try {
            const model: InputUnitListCreateModel = await this._validator.validateCreateRequest(request);
            const record = await this._service.create(model);
            if (record === null) {
                ErrorHandler.throwInternalServerError('Unable to add input unit list!', new Error());
            }
    ...
    update = async (request: express.Request, response: express.Response) => {
        try {
            ...
            const model: InputUnitListUpdateModel = await this._validator.validateUpdateRequest(request);
    ...
    search = async (request: express.Request, response: express.Response) => {
        try {
            const filters: InputUnitListSearchFilters = await this._validator.validateSearchRequest(request);
