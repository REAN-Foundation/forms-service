...
import { ValidationRuleCreateModel, ValidationRuleUpdateModel, RuleSearchFilters } from '../../../domain.types/forms/rule.domain.types';
...
export class ValidationRuleController extends BaseController {
    ...
    createValidationRule = async (request: express.Request, response: express.Response) => {
        try {
            const model: ValidationRuleCreateModel = await this._validator.validateValidationRuleCreateRequest(request);
            const record = await this._service.createValidationRule(model);
            if (record === null) {
                ErrorHandler.throwInternalServerError('Unable to create Validation Rule!', new Error());
            }
    ...
    getValidationRuleById = async (request: express.Request, response: express.Response) => {
        try {
    ...
    updateValidationRule = async (request: express.Request, response: express.Response) => {
        try {
            const id = await this._validator.validateParamAsUUID(request, 'id');
            const model: ValidationRuleUpdateModel = await this._validator.validateValidationRuleUpdateRequest(request);
            const updatedRecord = await this._service.updateValidationRule(id, model);
    ...
    deleteValidationRule = async (request: express.Request, response: express.Response): Promise<void> => {
        try {
            const id: uuid = await this._validator.validateParamAsUUID(request, 'id');
    ...
    searchValidationRule = async (request: express.Request, response: express.Response) => {
        try {
            const filters: RuleSearchFilters = await this._validator.validateRuleSearchRequest(request);
            const searchResults = await this._service.searchValidationRule(filters);
