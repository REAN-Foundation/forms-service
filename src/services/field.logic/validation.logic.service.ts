...
export class ValidationLogicService {
    constructor(@inject('IValidationLogicRepo') private _validationLogicRepo: IValidationLogicRepo) { }

    // Validation Logic operations
    async createValidationLogic(model: ValidationLogicCreateModel): Promise<ValidationLogicResponseDto> {
        return await this._validationLogicRepo.createValidationLogic(model);
    }

    async updateValidationLogic(id: string, model: ValidationLogicUpdateModel): Promise<ValidationLogicResponseDto> {
        return await this._validationLogicRepo.updateValidationLogic(id, model);
    }

    async getValidationLogicById(id: string): Promise<ValidationLogicResponseDto> {
