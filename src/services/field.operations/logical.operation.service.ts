...
export class LogicalOperationService {
    constructor(
        @inject('ILogicalOperationRepo')
        private _logicalOperationRepo: ILogicalOperationRepo
    ) {}

    // Logical Operation operations
    async createLogicalOperation(
    ...
    async updateLogicalOperation(
        id: string,
        model: LogicalOperationUpdateModel
    ): Promise<LogicalOperationResponseDto> {
        return await this._logicalOperationRepo.updateLogicalOperation(
            id,
            model
        );
    }

    async getLogicalOperationById(
        id: string
    ): Promise<LogicalOperationResponseDto> {
