...
export class MathematicalOperationService {
    constructor(
        @inject('IMathematicalOperationRepo')
        private _mathematicalOperationRepo: IMathematicalOperationRepo
    ) {}

    // Mathematical Operation operations
    async createMathematicalOperation(
    ...
    async updateMathematicalOperation(
        id: string,
        model: MathematicalOperationUpdateModel
    ): Promise<MathematicalOperationResponseDto> {
        return await this._mathematicalOperationRepo.updateMathematicalOperation(
            id,
            model
        );
    }

    async getMathematicalOperationById(
        id: string
    ): Promise<MathematicalOperationResponseDto> {
        return await this._mathematicalOperationRepo.getMathematicalOperationById(
            id
        );
    }

    async deleteMathematicalOperation(id: string): Promise<boolean> {
        return await this._mathematicalOperationRepo.deleteMathematicalOperation(
            id
        );
    }

    async searchMathematicalOperation(
        filters: OperationSearchFilters
    ): Promise<any> {
        return await this._mathematicalOperationRepo.searchMathematicalOperation(
            filters
        );
    }
}
