...
export class ResponseRepo extends BaseRepo implements IResponseRepo {
    _responseRepo: Repository<QuestionResponse> =
        Source.getRepository(QuestionResponse);
    ...
    getAll = async (): Promise<QuestionResponseResponseDto[]> => {
        try {
            ...
            ErrorHandler.throwDbAccessError(
                'DB Error: Unable to get Llm prompt record!',
                error
            );
        }
    };
    ...
    exportCsv = async (): Promise<string> => {
        ...
        const csvWriter = createObjectCsvWriter({
            ...
            header: [
                ...
                {
                    id: 'FormSubmission.SubmissionTimestamp',
    ...
    exportPdf = async (): Promise<string> => {
        ...
        doc.text(
            'ID\tSubmission ID\tTemplate ID\tForm URL\tUser ID\tStatus\tSubmission Timestamp\tCreated At\t' +
        ...
        data.forEach(row => {
            doc.text(
                `${row.id}\t${row.FormSubmission.id}\t${row.FormSubmission.TemplateId}\t${row.FormSubmission.FormUrl}\t${row.FormSubmission.UserId}\t${row.FormSubmission.Status}\t${row.FormSubmission.SubmissionTimestamp}\t${row.FormSubmission.CreatedAt}\t` +
