...
export class FormTemplateRepo extends BaseRepo implements IFormTemplateRepo {
    _formTemplateRepo: Repository<FormTemplate> =
        Source.getRepository(FormTemplate);

    _formField: Repository<FormFieldEntity> =
        ...
        const mapSections = async (
            ...
            return Promise.all(
                sections.map(async section => {
                    ...
                    sectionDto.Questions = questions.map(q =>
                        FormFieldMapper.toDto(q)
                    );
    ...
    search = async (filters: FormTemplateSearchFilters): Promise<any> => {
        try {
            ...
            const [list, count] =
                await this._formTemplateRepo.findAndCount(search);
    ...
    private getSearchModel = (filters: FormTemplateSearchFilters) => {
        ...
        if (filters.DefaultSectionNumbering) {
            search.where['DefaultSectionNumbering'] =
                filters.DefaultSectionNumbering;
        }
