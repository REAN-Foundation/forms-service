...
export class FunctionExpressionOperationRepo extends BaseRepo implements IFunctionExpressionOperationRepo {

    _functionExpressionOperationRepo: Repository<FunctionExpressionOperationEntity> = Source.getRepository(FunctionExpressionOperationEntity);
    
    // Function Expression Operation operations
    createFunctionExpressionOperation = async (model: FunctionExpressionOperationCreateModel): Promise<FunctionExpressionOperationResponseDto> => {
        try {
    ...
    updateFunctionExpressionOperation = async (id: string, model: FunctionExpressionOperationUpdateModel): Promise<FunctionExpressionOperationResponseDto> => {
        try {
            const updateData = await this._functionExpressionOperationRepo.findOne({
                where: {
                    id: id,
                    DeletedAt: null,
                },
            ...
            if (!updateData) {
                ErrorHandler.throwNotFoundError("Function Expression Operation Data not found!");
            }
    ...
    getFunctionExpressionOperationById = async (id: string): Promise<FunctionExpressionOperationResponseDto> => {
        try {
    ...
    deleteFunctionExpressionOperation = async (id: string): Promise<boolean> => {
        try {
            const record = await this._functionExpressionOperationRepo.findOne({
                where: {
                    id: id,
    ...
    searchFunctionExpressionOperation = async (filters: OperationSearchFilters): Promise<any> => {
        try {
            var search = this.getSearchModel(filters);
            var { search, pageIndex, limit, order, orderByColumn } =
                this.addSortingAndPagination(search, filters);
            const [list, count] = await this._functionExpressionOperationRepo.findAndCount(search);
