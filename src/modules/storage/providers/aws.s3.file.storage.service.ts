...
export class AWSS3FileStorageService {
    ...
    exists = async (storageKey: string): Promise<string> => {
        try {
            ...
            Logger.instance().error(error, 500, 'file does not exist');
            return null;
        }
    };
    ...
    uploadStream = async (storageKey: string, stream: Readable): Promise<string> => {
        ...
        try {
            ...
            Logger.instance().error(error, 500, 'Error in uploadstream');
        }
    };
    ...
    upload = async (storageKey: string, localFilePath?: string): Promise<string> => {
        ...
        try {
            ...
            Logger.instance().error(error.message, 500, 'error in uploading');
        }
    };
