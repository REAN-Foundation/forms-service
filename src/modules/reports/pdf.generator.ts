...
export class PDFGenerator {
    static getAbsoluteFilePath = async filePrefix => {
        ...
        var dateFolder = TimeHelper.getDateString(
            new Date(),
            DateStringFormat.YYYY_MM_DD
        );
    ...
    static addOrderFooter = (document, model, y, logoImagePath) => {
        ...
        document.image(imageFile, 0, 800, { width: 595 });

        document.fontSize(8).fillColor('#ffffff');
        if (model.ClinicName != null) {
            document.text(model.ClinicName, 100, 810, { align: 'right' });
        }

        document.fontSize(7);
        if (model.ClinicAddress != null) {
            document.text(model.ClinicAddress, 100, 822, { align: 'right' });
        }
    ...
    static addOrderMetadata = (y, document, model) => {
        ...
        var imageFile = path.join(
            process.cwd(),
            './assets/images/Prescription_symbol.png'
        );
        document.image(imageFile, 50, y, {
            width: 25,
            height: 25,
            align: 'left',
        });
        ...
        document.fillOpacity(1.0).lineWidth(1).fill('#444444');

        document.fillColor('#444444').font('Helvetica').fontSize(10);
